...
def save_plot_publications_by_year(data):
    ...
    ax = sns.barplot(x=year_counts.index, y=year_counts.values, color='skyblue', order=year_counts.index)
...
def save_plot_papers_by_type(data):
    ...
    ax = sns.countplot(data=data, x='Item Type', order=data['Item Type'].value_counts().index, color="brown")
    plt.ylabel("Number of Papers", fontsize=16)
...
def save_plot_authors_per_paper(data):
    ...
    plt.figure(figsize=(12, 8))
    ax = sns.barplot(x=author_count_counts.index, y=author_count_counts.values, color="purple", order=author_count_counts.index)
    plt.ylabel("Number of Papers", fontsize=16)
...
def save_plot_top_publishers(data):
    ...
    plt.figure(figsize=(12, 8))
    sns.countplot(data=filtered_data, y='Publication Title', order=top_publishers, color="yellow")
